...
blogController.get('/getAll', async (req, res) => {
    try {
        const blogs = await Blog.find({}).populate("userId", '-password')  // populate fetch the data mongodb -password', it specifies to exclude the 'password' field from the referenced data, likely for security reasons.
